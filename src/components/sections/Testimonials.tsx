...
export const Testimonials = () => {
    return (
        <section id="testimonials" className="bg-white text-slate-800 py-16 md:py-24 relative border-b border-slate-200/50">
            <div className="container">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {testimonialsData.map((testimonial, index) => (
                        <motion.div
                            key={testimonial.id}
                            className="group relative p-6 rounded-2xl glass-card-light flex flex-col h-full hover:border-violet-500/20"
                        ...
                        >
                            ...
                            <div className="absolute top-6 right-6">
                                <FaQuoteRight size={24} className="text-slate-900 opacity-5 transition-all duration-300 group-hover:text-violet-400 group-hover:opacity-20" />
                            </div>

                            {/* Stars */}
                            <div className="flex gap-1 mb-4">
                                {[...Array(5)].map((_, i) => (
                                    <FaStar key={i} className="text-violet-600 text-sm" />
                                ))}
                            </div>

                            <p className="text-slate-600 text-sm leading-relaxed italic mb-6 flex-grow">"{testimonial.quote}"</p>

                            <div className="flex items-center gap-4 border-t border-slate-200/60 pt-4 mt-auto">
                                ...
                                <div className="flex flex-col">
                                    <h4 className="font-heading text-base font-bold text-slate-900 mb-0.5">{testimonial.author}</h4>
                ...
                <div className="flex justify-center gap-2 mt-12">
                    ...
                    <span className="w-2 h-2 rounded-full bg-slate-200 cursor-pointer transition-all duration-300 hover:bg-slate-300"></span>
                    <span className="w-2 h-2 rounded-full bg-slate-200 cursor-pointer transition-all duration-300 hover:bg-slate-300"></span>
                </div>
            </div>
        </section>
